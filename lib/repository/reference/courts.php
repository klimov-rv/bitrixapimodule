...
class Courts extends ReferenceRepository
{
    public function getAll(array $params): array
    {
        ...
        $arSelect = array("ID", "NAME", "PROPERTY_COURT_ADDRESS"); 
        // выборка по типу справочника - $params["reference_type"]
         
        $blockEls = \CIBlockElement::GetList(
            array("ID" => "DESC"),  
            array("ACTIVE" => "Y", "IBLOCK_CODE" => 'courts'),  
            false, false, $arSelect);

        while ($element = $blockEls->fetch()) {
            array_push($result,  $this->prepareReturn($element, self::TYPE_DETAIL));
        }
        ...
        //         // }
        //         \Bitrix\Main\Diag\Debug::dumpToFile($element, $varName = '$element', $fileName = 'dumpToFile.txt');
